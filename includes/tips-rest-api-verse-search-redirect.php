...
class List_Tips_Verse_Search_Redirct_Rest_API extends WP_REST_Controller {
    ...
    public function __construct() {
        ...
        $this->base = 'bible/find-verse-red';
        $this->api_version = '1';
    ...
    public function find_verse_data(WP_REST_Request $request) {
        ...
        if (empty($chapter_verse[1])) {
            ...
            foreach ($terms as $verse) {
                ...
                $book_key = get_term_meta( $term_id, '_term_book_key', true ); 
    ...
    private function handle_no_verses_found($book) {
        ...
        return [(object) ['error' => $book]];
    }

    private function build_chapter_response($verses, $books_a, $book) {
        $post_object[] = (object) ['find_verse' => $book];
        return $post_object;
    }
}
$lps_rest_server = new List_Tips_Verse_Search_Redirct_Rest_API();
